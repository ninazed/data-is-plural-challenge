...
ladyquote <- filter(quotecluster, speaker == 1)
manquote <- filter(quotecluster, speaker == 4)  
bothquotes <- rbind(ladyquote, manquote) #bind them back together
bothquotes$speaker <- as.character(bothquotes$speaker)
lessquote <- revalue(bothquotes$speaker, c("1" = "0", "4" = "1"))
...
print (data_mod)
#do vis
p1 <- ggplot(data = q2) + aes(y = lessquote, x = bothquotes.quoteType) + geom_jitter(width = 0.35, height=0.4, size = 8, shape = "♥", alpha = 0.5, aes(color=factor(lessquote)), show.legend = FALSE) + scale_x_continuous( breaks = c(0, 1, 2), label = c("Anaphoric", "Explicit", "Implicit")) + scale_y_continuous( breaks = c(0, 6), label = c("Emma", "Mr Knightley")) + scale_shape_manual(values=c(0, 6)) + scale_color_manual(values=c('#FF5733','#3347FF')) 
